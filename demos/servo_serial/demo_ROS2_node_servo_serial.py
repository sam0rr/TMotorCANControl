...
class MotorNode(Node):
    def __init__(self, device):
        ...
        self.velocity_sub = self.create_subscription(
            Float32, f"{self.dev_name}/velocity", self.velocity_cb, 10
        )

        self.duty_sub = self.create_subscription(
            Float32, f"{self.dev_name}/duty", self.duty_cb, 10
        )

        self.position_sub = self.create_subscription(
            Float32, f"{self.dev_name}/position", self.position_cb, 10
        )

        self.current_sub = self.create_subscription(
            Float32, f"{self.dev_name}/current", self.current_cb, 10
        )

        self.output_pub = self.create_publisher(
            Float32MultiArray, f"{self.dev_name}/output", 10
        )
