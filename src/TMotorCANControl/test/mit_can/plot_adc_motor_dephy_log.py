...
angle_dephy = (
    (np.array(angle_dephy) / gearbox_gear_ratio) * tmotor_gear_ratio * num_pole_pairs
)

angle_dephy_func = interpolate.interp1d(
    time_dephy, angle_dephy, kind="linear", fill_value="extrapolate"
)
...
torque_flucuation_magnitude = (
    torque_adc_adjusted[time < 30].max() - torque_adc_adjusted[time < 30].min()
) / np.average(torque_adc_filtered)
...
num_cycles = 5
plt.plot(
    angle_dephy_adjusted[angle_dephy_adjusted < 3.14 * 2 * num_cycles],
    torque_adc_adjusted[angle_dephy_adjusted < 3.14 * 2 * num_cycles],
    label="(worst torque fluctuation: "
    + str(round(torque_flucuation_magnitude, 2))
    + "%)",
)
