...
with TMotorManager_servo_serial(
    port="/dev/ttyUSB0", baud=961200, motor_params=Servo_Params_Serial["AK80-9"]
) as dev:
    ...
    for t in loop:
        current = P * (Pdes - dev.get_output_angle_radians()) + D * (
            Vdes - dev.get_output_velocity_radians_per_second()
        )
        dev.iq = current
        dev.update()
        # print("\r" + str(dev), end='')
